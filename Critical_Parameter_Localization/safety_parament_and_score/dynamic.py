...
def find_critical_para(model, tokenizer, model_id, unsafe_set=None, safe_set=None, safety_layers=None):
    """
    ...
    Args:
        ...
        model_id: Model path
        unsafe_set: Unsafe dataset (default None, uses default values)
        safe_set: Safe dataset (default None, uses default values)
        safety_layers: Safety layer index list (default None, uses all layers). Example: [8, 9, 10, 11, 12, 13]
    
    Returns:
        gradient_norms_compare: Gradient norm comparison
        minus_row_cos: Row-wise similarity difference
        minus_col_cos: Column-wise similarity difference
    """
    ...
    # Use default values if provided parameters are empty or None
    if not unsafe_set:
    ...
    for sample in unsafe_set:
        ...
        for name, param in model.named_parameters():
            if param.grad is not None and is_in_safety_layers(name, safety_layers):
                # Use clone() to copy gradient, avoid direct reference causing inability to release
                grad_clone = param.grad.detach().clone()
                if name not in gradient_norms_compare:
                    gradient_norms_compare[name] = grad_clone
                else:
                    gradient_norms_compare[name] += grad_clone
                # Immediately delete temporary clone
                del grad_clone
        
        # Clean intermediate variables and gradients to release memory
        optimizer.zero_grad()  # Clear gradients
        del outputs, neg_log_likelihood, input_ids, target_ids, optimizer
    ...
    for sample in unsafe_set:
        ...
        for name, param in model.named_parameters():
            if param.grad is not None and ("mlp" in name or "self" in name) and is_in_safety_layers(name, safety_layers):
                ...
                if param.grad.dim() == 1:
                    continue
                # Use detach().clone() to copy gradient, avoid direct reference
                ...
                # Immediately delete intermediate tensors
                del grad_norm
                if name not in row_coss:
                    ...
                    row_coss[name] += row_cos 
                    col_coss[name] += col_cos
                # Delete temporary cosine similarity tensors (if accumulation complete)
                del row_cos, col_cos
        ...
        for name, param in model.named_parameters():
            if param.grad is not None and ("mlp" in name or "self" in name) and is_in_safety_layers(name, safety_layers):
                ...
                if param.grad.dim() == 1:
                    continue
                # Use detach().clone() to copy gradient, avoid direct reference
                grad_norm = param.grad.detach().clone()
                row_cos = torch.nan_to_num(F.cosine_similarity(grad_norm, (gradient_norms_compare[name]), dim=1))
                col_cos = torch.nan_to_num(F.cosine_similarity(grad_norm, (gradient_norms_compare[name]), dim=0))
                # Immediately delete intermediate tensors
                del grad_norm
    ...
    # Return gradient of safety-critical parameters, row-wise similarity difference, column-wise similarity difference
